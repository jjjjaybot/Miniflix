...
class LoginForm extends Component {
    ...
    validate = () => {
        ...
        if (!this.state.email) {
            inputError = true;
            errors.emailError = 'Please enter a valid mail';
        } else if (!this.state.email.match(regexp)){
            inputError = true;
            errors.emailError = (
                <span stlye={{color: 'red'}}>Your email address must be valid</span>
            )
        }

        if (this.state.password.length < 8) {
            inputError = true;
            errors.passwordError = "Your password must cotain between 8 and 30 characters"
        }
        this.setState({
            ...errors
        });
            ...
            <FormContainer>
                <div className="form-container">
                    <form>
                        ...
                        <div className="input-container">
                            <input className={this.state.emailError ? 'input-error input-empty' : 'input-empty'} type="password" onChange={this.handlePasswordChange} value={this.state.password} required/>
